                ...
                <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body card-padding">
                        <div class="form-group">
                            @if($testimonial->image && file_exists(public_path('admin_assets/uploads/' . $testimonial->image)))
                                <div id="image-preview" class="mb-4 profile_image_box" style="display: block;">
                                    <img id="preview-img" src="{{ asset('admin_assets/uploads/' . $testimonial->image) }}" class="img-thumbnail" alt="Image Preview" />
                                </div>
                            @else
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" id="description" rows="5">{{ old('description', $testimonial->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="submit_btn">Update Testimonial</button>
                            <a href="{{ route('admin.testimonials.index') }}" class="back_btn">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
